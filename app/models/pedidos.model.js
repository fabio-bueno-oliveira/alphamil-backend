...
Pedido.getAll = result => {
  sql.query("SELECT p.id, p.id_pedido_impresso, c.nome AS cliente, p.data_pedido_realizado, p.data_entrega_realizada_1, p.condicoes_de_pagto, COUNT(i.id) AS itens, SUM(i.quantidade) AS quantidade_total_itens, e.nome AS status_entrega FROM `pedidos` AS p LEFT JOIN pedidos_itens AS i ON p.id = i.id_pedido LEFT JOIN clientes AS c ON p.id_cliente = c.id LEFT JOIN entrega_status_nomes AS e ON p.status_entrega = e.id", (err, res) => {
    if (err) {
